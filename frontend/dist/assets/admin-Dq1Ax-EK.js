import{d as h,e as u,f as g,h as v}from"./api-AaxENaXw.js";import{e as b}from"./admin-auth-x3miNVaR.js";async function y(){await b();const a=document.getElementById("cards"),s=document.getElementById("latest-products"),r=document.getElementById("latest-pages"),l=document.getElementById("logout-btn");if(l&&l.addEventListener("click",async e=>{e.preventDefault();try{await h(),window.location.href="admin-login.html"}catch{window.location.href="admin-login.html"}}),!(!a||!s||!r))try{const[e,i,m]=await Promise.all([u().catch(()=>({data:{}})),g({per_page:20}).catch(()=>({data:[]})),v().catch(()=>({data:[]}))]),d=e.data||e||{},n=Array.isArray(i.data)?i.data:[],c=Array.isArray(m.data)?m.data:[];a.innerHTML=`
